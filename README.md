# ApexPlanet Cybersecurity & Ethical Hacking Internship

## Intern Information

* **Name:** VANAM CHENCHU PUNEETH
* **College:** Siddharth Institute of Engineering & Technology, Puttur
* **Domain:** Cybersecurity & Ethical Hacking

## Internship Tasks

### Task 1 - Foundation and Environment Setup

✅ Completed and Submitted

### Task 2 - Network Security & Scanning

✅ Completed and Submitted

### Task 3 - Web Application Security

✅ Completed and Submitted

### Task 4 - Exploitation & System Security

🔄 In Progress

### Task 5 - Capstone Project & Incident Response

⏳ Pending

## Repository Structure

```text
Task-1-Foundation-and-Environment-Setup/
Task-2-Network-Security-and-Scanning/
Task-3-Web-Application-Security/
Task-4-Exploitation-and-System-Security/
Task-5-Capstone-Project-and-Incident-Response/
```

## Internship Overview

This repository contains all reports, screenshots, notes, cheat sheets, and practical work completed during the ApexPlanet Cybersecurity & Ethical Hacking Internship Program. The internship covers cybersecurity fundamentals, network security, web application security, penetration testing, vulnerability assessment, incident response, and capstone project implementation.

## Status

✅ Task 1 Submitted Successfully

✅ Task 2 Submitted Successfully

✅ Task 3 Submitted Successfully

